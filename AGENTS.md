...
- `modules/agent-service`：后台 agent loop / runtime worker，消费 queue、执行主 agent run、维护 delivery state，并运行 presence 后台循环和 self-action eligibility 入口
...
- agent run、行为判断、delivery state、自学习、presence tick、self-action eligibility skip、历史数字行动投影和后台任务执行问题：看 `modules/agent-service`
