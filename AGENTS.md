...
- `modules/agent-service`：后台 agent loop / runtime service，消费 Notify Bucket、执行主 agent run、路由模型 response action、维护 delivery state，提供 `$qq-usage` 工程 API，并维护 life event 投影；当前没有旧式固定 presence runner。runtime 伪代码和边界只维护在 `docs/XIAONI_AGENT_STACK_LEDGER.md`
