...
- 当前 agent loop 宿主仍是 `agent-service`；`agent_queue_messages` 是 Notify Bucket 的持久化门铃，不是小腻的 QQ app 未读列表、认知边界或长期运行凭证。主 loop、no-notify、`self_continuation` 和 request 组装细节只看 `docs/XIAONI_AGENT_STACK_LEDGER.md`
- 当前底层事实源是追加式 Xiaoni agent stack：`agent_stack_items` 记录连续可回放上下文，`llm_request_slices` 记录真实 LLM 请求和 provider wire payload，`tool_executions` 记录工具执行；完整 canonical/wire request/response 由 `provider-service` / Codex Provider 写入 `llm_request_slices`，`agent-service` 只组装 canonical request 并回填 slice 的 stack index；行动流是这些事实的投影。旧 `llm_call_logs`、`tool_execution_logs`、provider replay ledger 已移除并由 schema ensure drop。细节看 `docs/XIAONI_AGENT_STACK_LEDGER.md`
...
- `modules/agent-service`：后台 agent loop / runtime service，消费 Notify Bucket、组装主 agent canonical request、路由模型 response action、维护 delivery state，提供 `$qq-usage` 工程 API，并维护 life event 投影；完整 LLM request/response 由 `provider-service` 记录，agent-service 只回填 stack link；当前没有旧式固定 presence runner。runtime 伪代码和边界只维护在 `docs/XIAONI_AGENT_STACK_LEDGER.md`
...
- 在任务 worktree 中工作时，数据库必须连接主工作区对应的主栈 DB 资源。不要让 compose volume、`DATABASE_URL`、本地脚本或测试容器隐式创建或连接 worktree 私有 DB；涉及 DB 的启动、重建、迁移、dump、restore 前，先确认 Postgres 容器挂载和连接目标指向 `/home/liahua/IdeaProject/qq_bot` 的主工作区资源。
