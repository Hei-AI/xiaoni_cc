...
class HttpServer {
    ...
    async handleGetTokenStats(req, res) {
        try {
            ...
            const dailyLogs = await this.database.executeQuery(`
        SELECT 
     ...
     * 获取Token使用历史统计 - 新增
     */
    async handleGetTokenUsageHistory(req, res) {
        try {
      ...
      `;
            ...
            const logs = await this.database.executeQuery(logsQuery, queryParams);
            // 获取总数
            const totalQuery = await this.database.executeQuery(`SELECT COUNT(*) as count FROM api_token_logs l ${whereClause}`, params);
            // 获取摘要统计
            const summaryQuery = await this.database.executeQuery(`
        SELECT 
          COUNT(*) as total_requests,
          SUM(CASE WHEN result = 'success' THEN 1 ELSE 0 END) as successful_requests,
          SUM(CASE WHEN result = 'error' THEN 1 ELSE 0 END) as failed_requests,
    ...
    async handleUpdateGroup(req, res) {
        try {
            ...
            if (auto_reply_enabled !== undefined)
                updateData.auto_reply_enabled = Boolean(auto_reply_enabled);
     ...
     */
    async handleBulkGroupOperations(req, res) {
        try {
            const { group_ids, action, settings } = req.body;
     ...
     */
    async handleGetGroupStats(req, res) {
        try {
