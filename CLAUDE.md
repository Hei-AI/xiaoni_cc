...
- 近期状态、待修复项和自验证流程请查看 `docs/PROJECT_STATUS.md`；路线图参见 `docs/ROADMAP.md`。
...
更多架构背景：`docs/HUMAN_LIKE_PROCESSOR_FLOW.md`、`docs/LLM_TOOL_EXECUTION_DESIGN.md`。
...
完整部署指引参见根目录 `DOCKER.md`。

## 4. 数据库与网络
- 连接参数示例：`mysql -u qqbot_user -pqqbot_password -h localhost -P 3306 qqbot_db`。
- 主要表：`conversations`、`llm_call_logs`、`llm_jobs` 等，迁移脚本位于 `database/migrations/`。
- 服务默认在 Docker 网络 `qq_bot_network` 中运行，容器名称可以互相解析。
- HTTP 透明代理及运维脚本位于 `modules/http-traffic-monitor/transparent-proxy/`，详细说明见 `docs/archive/MITMPROXY_STARTUP_GUIDE.md`。

## 5. AI 配置与工具编排
- 管理端提供实时配置接口，启用/验证流程参见 `docs/PROJECT_STATUS.md` 中的自验证章节。
- LLM 工具系统组件：`LLMJobWorker`、`FunctionCallDispatcher`、`ToolRegistryService`、`tools/static-tools.ts`。
- 相关设计与数据库说明：`docs/LLM_TOOL_EXECUTION_DESIGN.md`、`database/migrations/010_create_llm_tool_system_tables.sql`。
...
- 推荐流程：
  ...
  ```
- 目前已提供的模块文档：`modules/qqbot-core/CLAUDE.md`、`modules/http-traffic-monitor/CLAUDE.md`。

如遇到架构或流程不明确的情况，优先查阅 `docs/PROJECT_STATUS.md` 与对应专题文档，必要时与维护者同步确认后再改动。
