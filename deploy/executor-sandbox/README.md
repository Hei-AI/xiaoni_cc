...
- C 宿主 cli-proxy(host 8317/18317)→ iptables INPUT 拦 br-xexec 到宿主端口(只留 9977)。
- D docker.sock 偷各服务 key 后走开放外网 → 删 docker.sock 挂载(执行容器自身 env/挂载无 key,唯一偷法就是 sock)。
- 额外(codex 对抗抓到,决定性):9977 浏览器桥驱动的是操作者真登录态镜像 profile,登录着 claude.ai/chatgpt/gemini。
  程序可用 goto/run-code 脚本化这些 UI 白嫖,绕过全部网络封锁 → 桥入口按域名硬拦(xiaoni_playwright_cli_bridge.py 的 _MODEL_UI_DENY_HOSTS)。
...
- modules/agent-service/skills/xiaoni-browser/scripts/xiaoni_playwright_cli_bridge.py:模型 UI 导航拦截。
- deploy/executor-sandbox/:firewall 脚本 + systemd unit/timer、deploy.sh、verify.sh。

## 保留 / 已知残留
- notify → 唤醒她本人主 agent、她醒着自己选择答访客 = 她的意志,非程序白嫖。保留。
- 重启空窗:host 重启后 docker 可能在防火墙 unit 前拉起执行容器(秒级)。timer 每分钟重装 + DOCKER-USER/自建链 docker 不 flush 兜底。
