...
SET
    system_instructions = JSON_ARRAY(
...
9. Use the metadata and tool schemas to infer execution parameters. Do not ask for IDs that are already present in context.'
    ...
    description = '基础聊天机器人配置（按 2026-03-24 最近运行快照恢复）',
    config_version = 'v1.0',
    last_config_update = CURRENT_TIMESTAMP,
    is_active = TRUE,
    version = GREATEST(COALESCE(version, 1), 3),
    updated_at = CURRENT_TIMESTAMP
WHERE agent_type = 'chat_bot'
  AND prompt_name = 'basic_chat';
