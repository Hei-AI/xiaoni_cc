...
- 2026-06-11 历史污染清理：以旧文本 `当前连续生命切片已经进入上下文` 为 marker，删除 `xiaoni_replay_events` 24 行（id `2655..2701`）和 `llm_call_logs` 24 行（id `12380..12403`）。备份位于 `.tmp/xiaoni-cleanup/`；删除后全库 text/json 扫描该 marker 为 0 命中。
- 2026-06-11 追加历史污染清理：清理重启观察前仍会回放进新请求的历史 runtime tag。删除 1 条 `conversation_items` runtime-only item；净化 209 条 `conversations.raw_response.responses_replay_items`，只剥离历史 `role=assistant` 文本中的 `<system_reminder>` / 旧 `<STATE...>`，保留其余 OS / action 文本。备份：`.tmp/xiaoni-cleanup/conversation_items_assistant_runtime_tags_20260611_022219.jsonl`、`.tmp/xiaoni-cleanup/conversations_replay_runtime_tags_20260611_022219.jsonl`。
- 2026-06-11 观察期脏数据清理：删除清源前重启观察产生的 1 条 `conversation_items` phone-notification reminder、4 条 `llm_call_logs`（id `12404..12407`）和 4 条 `xiaoni_replay_events`（id `2703,2705,2707,2709`）。备份：`.tmp/xiaoni-cleanup/conversation_items_any_runtime_tags_20260611_022705.jsonl`、`.tmp/xiaoni-cleanup/llm_call_logs_post_clean_dirty_20260611_022705.jsonl`、`.tmp/xiaoni-cleanup/xiaoni_replay_events_post_clean_dirty_20260611_022705.jsonl`。
- 2026-06-11 legacy 请求日志净化：为避免管理面“最近 LLM 请求”继续显示更早的历史污染样本，压缩备份并原地净化历史 request input；`llm_call_logs` 更新 2432 条，`xiaoni_replay_events` 更新 670 条。净化范围只限 request `input[]` 中 `role=assistant` 的 content text，移除 `<system_reminder>` / 旧 `<STATE...>`，不处理当前合法的 developer prompt / tool description。备份：`.tmp/xiaoni-cleanup/llm_call_logs_legacy_assistant_runtime_tags_20260611_023434.jsonl.gz`、`.tmp/xiaoni-cleanup/xiaoni_replay_events_legacy_assistant_runtime_tags_20260611_023434.jsonl.gz`。
- 2026-06-11 最终校验：`llm_call_logs` / `xiaoni_replay_events` 中 `role=assistant` 的 runtime tag 命中为 0；`conversation_items` runtime tag 为 0；`conversations.raw_request/raw_response` runtime tag 为 0；旧 marker `当前连续生命切片已经进入上下文` 在 LLM/replay 请求中为 0。runtime 已重新开启，`curl -fsS http://127.0.0.1:8092/health` 返回 healthy，12 个 15 秒采样内无新 LLM/replay、队列计数不变。
- 2026-06-11 追加源头防回写：`buildInboundBatchTranscriptItems()` 对 `phone_notification` / `image_task_notification` / `system_reminder` / `self_continuation` 统一不生成 `conversation_items`；`createConversation.userMessage` 对这些 runtime reminder 统一写空字符串。runtime reminder 只作为当前输入进入模型，不作为 QQ 正文或 assistant/user 历史回放。
- 2026-06-11 追加 `conversations.user_message` 清理：发现并备份 2 条 runtime-only user_message（conversation `6563`、`6564`），已置空以避免管理面或后续读取把它当用户正文。备份：`.tmp/xiaoni-cleanup/conversations_user_message_runtime_tags_20260611_025535.jsonl`。复查结果：`llm_call_logs` / `xiaoni_replay_events` 中 `role=assistant` runtime tag 命中为 0；`conversation_items` runtime tag 为 0；`conversations.user_message/raw_request/raw_response` runtime tag 为 0；旧 marker 在 `agent_queue_messages`、LLM/replay、conversation 表中均为 0。
- 2026-06-11 worktree 审计：`qq_bot-worktree-final-answer-idle-user-role`、`qq_bot-worktree-image-vision-fork-*`、`qq_bot-worktree-memory-participation-audit`、`qq_bot-worktree-action-stream-time-filter-ux`、`qq_bot-task10-image-routing` 等旧 worktree 仍包含 `FINAL_ANSWER_IDLE_REMINDER_TEXT` 或 `tool_loop_monitor` 旧实现，不能整分支 / 整文件覆盖合入当前 main。`task10-image-routing` 的图片 edit 无源降级逻辑当前 main 已有等价实现；`action-stream-time-filter-ux` 只可后续按 admin/persistence 相关文件手动挑选，不能带回旧 agent-service runtime 文件。
- 2026-06-11 验证补充：本次触碰的 runtime / docs / todo 文件 `git diff --check -- <paths>` 通过；全仓 `git diff --check` 当前被既有 dirty 文件 `resource/napcat_config/webui.json` 的 trailing whitespace 拦住，该文件不是本轮 runtime reminder 改动范围，未处理。
- 2026-06-11 宽标签补清：额外发现 2026-06-06/07 旧 `<PHONE_NOTIFICATION ... />` 以 `conversation_items.source='inbound_batch'` 形态进入历史，导致 2026-06-11 03:10 后的新 self-continuation 仍从旧 history 回放 `<PHONE_NOTIFICATION>` / `final_answer_turn_control` 字样。已修代码：历史 notification snapshot 过滤同时覆盖 `sensory_event` 和 `inbound_batch`；测试 `buildInitialInput does not replay historical notification snapshots as current input` 已补 `inbound_batch` 覆盖。
- 2026-06-11 宽标签数据清理：备份并删除 46 条旧 runtime notification `conversation_items`，置空 46 条旧 `conversations.user_message`，清空 12 条 `conversations.raw_response.responses_replay_items`；备份文件：`.tmp/xiaoni-cleanup/legacy_runtime_notification_conversation_items_20260611_031410.jsonl`、`.tmp/xiaoni-cleanup/legacy_runtime_notification_conversations_20260611_031410.jsonl`。同时备份并净化 `llm_call_logs` 398 行、`xiaoni_replay_events` 394 行，只从 request `input[]` 的 `role in ('assistant','user')` 中移除旧 runtime tag item；备份：`.tmp/xiaoni-cleanup/llm_call_logs_legacy_runtime_notification_items_20260611_031410.jsonl.gz`、`.tmp/xiaoni-cleanup/xiaoni_replay_events_legacy_runtime_notification_items_20260611_031410.jsonl.gz`。复查宽标签计数：LLM/replay user/assistant 输入、`conversation_items`、`conversations.user_message/raw_response` 均为 0。
- 2026-06-11 最终运行观察：重新 `docker compose build agent-service && docker compose up -d agent-service` 到 image `sha256:a0d0b124...`。部署后新产生 LLM/replay/conversation 到 `llm_call_logs=12488`、`xiaoni_replay_events=2871`、`conversations=6619`；采样期间 new user/assistant legacy runtime tag 计数保持 0，conversation 表宽标签保持 0。最近 self-continuation 都无 visible delivery，最后一条 conversation `6619` 主动 `recover_energy`，之后 4 次 15 秒采样 latest id 不变、queue pending/processing 为 0，health healthy 且 runtime_enabled/autonomous_runtime_enabled 都为 true。
