...
function extractCallResult(providerResponse) {
...
async function providerReplay(bodyOverride) {
  ...
  if (!response.ok || !payload?.success) {
    throw new Error(`Provider replay failed: HTTP ${response.status} ${JSON.stringify(payload).slice(0, 1000)}`);
  }
...
function buildPlanningBody(originalBody, instructionVariant) {
  const next = normalizeCanonicalRequest(originalBody);
...
async function runSingleStepControl(originalBody) {
  ...
  const parsed = extractCallResult(replay);
  return {
...
async function runTwoStageVariant(originalBody, instructionVariant) {
  ...
  const planningParsed = extractCallResult(planningReplay);
  ...
  const executionReplay = await providerReplay(executionBody);
  const executionParsed = extractCallResult(executionReplay);

  return {
    planning: {
      provider: planningReplay.provider || null,
      model: planningReplay.model || null,
    ...
    execution: {
      provider: executionReplay.provider || null,
      model: executionReplay.model || null,
...
async function main() {
  ...
  const outPath = path.join(
    ...
    'liahua-qq_bot',
    'replay',
    `traffic-${TRAFFIC_ID}-provider-plan-workflow-replay-${stamp}.json`
  );
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
  fs.writeFileSync(path.dirname(outPath) ? outPath : `./${path.basename(outPath)}`, `${JSON.stringify({
