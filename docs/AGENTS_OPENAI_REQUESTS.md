...
- single-flight 验收边界：只约束一个 provider-service 进程内、带相同缓存命名空间的请求；不能保证上游缓存一定命中，也不能把 HTTP 取消当作上游已停止计算/计费的证明。Anthropic `429` 的 `retry-after` 超过 30 秒时，本次调用直接返回错误，不在 provider 内等待数小时；该行为不是账号级熔断。限流期间若需要停止全部自动心跳，必须同时设置 `cache_heartbeat_paused=true` 和 `debug_cache_heartbeat_interval_ms=0`，只暂停主 loop 不够。
