...
- single-flight 验收边界：只约束一个 provider-service 进程内、带相同缓存命名空间的请求；不能保证上游缓存一定命中，也不能把 HTTP 取消当作上游已停止计算/计费的证明。Anthropic `429` 会记录按 base URL 共享的 Retry-After 截止时间（秒数或 HTTP 日期）；同入口的主请求、fork、小模型请求在每次上游尝试前等待到期，取消一个等待者不会清掉截止时间，后来的 429 只能延长等待。超过 30 秒的数字 Retry-After 仍结束当前失败调用，后续调用遵守共享截止时间，到期后主请求仍受 single-flight 约束。截止时间是本进程内存态，重启后可能有一次重新探测；不同账号若复用同一入口也会保守地一起等待。主运行开关与 provider 限流分离：429 不要求关闭主 loop。调用方超时可能重建内部请求，但截止时间前不会因此重发上游。若操作者确需停止全部自动心跳，须同时设置 `cache_heartbeat_paused=true` 和 `debug_cache_heartbeat_interval_ms=0`。
