...
- 不要把空闲行为做成第二套 planner 或硬编码兴趣表。当前空闲生活事件走 presence tick append 到同一条事件流；life-only tick 读取全局最近事件流切片，再由 main loop 决定内部 `submit_life_action`、`web_search` 或 `stay_silent`。没有具体 IM 目标时不能直接发 QQ，想回头分享的内容会留进 `<小腻的OS>`。
