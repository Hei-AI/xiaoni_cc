...
- 不要把空闲行为做成第二套 planner 或硬编码兴趣表。当前空闲生活事件走 presence tick append 到同一条事件流；presence 起源的 tick 读取全局最近事件流和 `xiaoni:global` 连续性，即使打开了游标后的未读 IM 并 materialize 成 `proactive_im_open`，也不能退回单个群/私聊的局部上下文。没有具体 IM 目标时不能直接发 QQ，只能由 main loop 决定内部 `submit_life_action`、`web_search` 或 `stay_silent`；想回头分享的内容会留进 `<小腻的OS>`。
