...
- **预算 4 个,严格优先级填充**(`placeCacheBreakpoints`):① system 头 → ② prevBoundary(确定性滑窗共享)→ ③ true-end tail(缓存冻结提示)→ ④ lastDurable(空闲 final 保暖 + 漂移兜底)。尾部集合最多 3 个不同点,所以 system + 尾部集合**恒 ≤ 4**:带提示的 3-点满帧正好占满 4,稳态无提示帧 tail==lastDurable 塌到 2,谁都不用丢。**已移除压缩头 `cache_anchor`**:压缩 fork 现在整体克隆主请求、直接吃全暖前缀,单独的 `[..H_X]` 断点没有读者;留着只会在带提示的压缩帧里把有用的 lastDurable 挤掉。
