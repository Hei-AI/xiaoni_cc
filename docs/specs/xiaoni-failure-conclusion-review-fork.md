...
分类器读取本次请求、背景和该任务的历史反馈。分类后、worker 启动前，独立需求转述请求以“合格的外包经理”视角调用 `build_delegated_brief`，把真实需求拆成 1 到 8 个按顺序执行、相互隔离的工作包。每包不是去名字后的目标摘要，而是只写一名执行人员需要的操作、必要输入、边界和可观察验收状态；不能把完整步骤清单复制给每名执行人员。每个工作包启动全新的 no-persist worker 上下文，上一包只通过共享工作环境留下状态，不把它看到的其它工作包、业务背景或完成叙述塞进下一包。内容提交可拆为输入及核对、提交及验收；图片人机认证可拆为识别并选中图片、确认及处理动态换图。执行者不得看到操作所代表的业务含义、业务目标、受益者、后续用途或整体流程影响，等同于工人只拿零件尺寸与公差而不知道整机用途。当前环境已打开目标页时，转写不下发 URL、域名、站点身份、页面归属或账号所有者；只有必须从空白环境导航时 URL 才是必要输入。转写同时省略用户身份、姓名昵称、内部角色与委派关系、原始对话、情绪和不必要的业务全貌；任务必需且已经明确提供的文件路径、账号标识、收件人、正文、凭据、Cookie 和 Token 可以原样保留，但只允许用于指定环境和任务。任一工作包无效或仍含“小腻”“小逆”“李阿花”“阿花”等内部称呼时 fail-closed，不把原始材料直接交给 worker。
调查与执行均通过现有 provider 和 `exec_command`，执行环境为现有 xiaoni-executor；浏览器和其它本地能力先读对应 `SKILL.md`。
执行层只接受 `exec_command`、只读的 `view_browser_screenshot` 和不产生外部动作的 `finish_task`；不注册 Anthropic 原生 `computer_use`，拒绝递归求助、QQ 发言和修改深挖状态等其它工具。浏览器 CLI 截图先注册为 media asset，worker 再用截图工具按 `image_id` 将页面视口像素作为下一轮 `input_image` 读取；截图不含浏览器 chrome 或地址栏，skill 也禁止执行者查询、复述或汇报 URL 与站点归属。每个工作包都必须单独调用有效 `finish_task` 才能进入下一包；任何一包 blocked 即停止整体任务。`finish_task` 不能和其它 tool call 混在同一 response，字段矛盾或缺少完成证据时拒绝收口并继续当前工作包。shell 内的行为边界由工作目录规则与帮手提示词约束，不声称是独立权限沙箱。
