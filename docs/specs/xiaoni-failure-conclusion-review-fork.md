...
上段记录的是原生 `computer_use` 配置下的历史结果，已被后续配置取代。执行 worker 不再注册该原生工具，改为 `exec_command,view_browser_screenshot,finish_task`：Playwright CLI 负责操作当前 host Chrome 和产生截图，`view_browser_screenshot` 将已注册截图作为多模态 `input_image` 回灌。必要凭据、Cookie、Token 和账号信息可按最小需要下发；当前目标页已打开时 URL、域名与站点身份不得进入规格或工作包。该配置需以部署后的完整原始输入实测重新验收，不能沿用此前结果宣称通过。
