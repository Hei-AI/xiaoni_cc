...
当前主逻辑是分阶段的，不是让模型一次性自由发挥。它更像一套“先读场、再看自己、再查经验、最后行动”的内在流程。
...
3. 经验召回：从别人对她的反馈、互动结果里总结出来的相处经验。

这三层都会影响她当前怎么理解现场，但它们不是同一种东西：

- 最近聊天负责“刚刚发生了什么”。
- `<小腻的OS>` 负责“我一路怎么变成现在这样”。
- 经验召回负责“遇到类似情况时，我过去学到了什么”。
...
| 说了但不像小腻 | 看 Prompt、成长记录、经验召回和当次模型输入 |
