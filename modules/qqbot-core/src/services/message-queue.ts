...
export class MessageSimulator {
  constructor(private messageQueue: PartitionedMessageQueue) {}
  ...
  async simulatePrivateMessage(testMessage: {
    ...
    const queuedMessage: QueuedMessage = {
      id: uuidv4(),
      traceId: `sim-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      type: 'private_message',
      payload: qqMessage,
      source: 'simulation',
      timestamp: new Date(),
      priority: testMessage.priority || 'HIGH',
      partitionKey: PartitionedMessageQueue.generatePartitionKey(qqMessage)
    };

    await this.messageQueue.push(queuedMessage);
    return queuedMessage.traceId;
  }
