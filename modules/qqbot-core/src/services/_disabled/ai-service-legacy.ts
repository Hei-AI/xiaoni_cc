...
import { ToolManager } from './predefined-tools';
import { logger } from '../utils/logger';
import { DatabaseManager } from './database';
import { getTokenManager } from '../utils/token-manager';
import { LoggingService } from './logging-service';
import { LegacyConfigConverter } from './config-converter';
import { v4 as uuidv4 } from 'uuid';

export class AIService implements ConfigurableLLMService {
  ...
  private currentGenAI: GoogleGenAI | null = null;
  private currentToken: string | null = null;
  private promptCache: Map<string, AgentPromptData> = new Map();
  private cacheTimeout: number = 5 * 60 * 1000; // 5 minutes
  // 🆕 高级配置缓存
  private advancedConfigCache: Map<string, AgentAdvancedConfig> = new Map();
  // 保留axios作为备用，用于健康检查等
  private baseURL = 'https://generativelanguage.googleapis.com/v1beta/models';

  // 🔥 新增：模型能力缓存
  private modelCapabilitiesCache: Map<string, any> = new Map();
  private configValidationCache: Map<string, any> = new Map();

  // 🔥 P0重构：统一配置系统
  private configConverter: LegacyConfigConverter;
  private unifiedConfigCache: Map<string, UnifiedLLMConfig> = new Map();
  private useUnifiedConfig: boolean = false; // 🔧 统一配置开关
  ...
  constructor(config: AIConfig, database: DatabaseManager, loggingService: LoggingService) {
    ...
    this.configConverter = new LegacyConfigConverter();
    // 异步初始化
    this.initializeGenAI().catch(error => {
      this.moduleLogger.error('AI service initialization failed', { error });
    });
    this.initializeDefaultPrompts();
    this.moduleLogger.info('AI Service initialized with unified configuration system');
  }
  ...
  private async initializeGenAI(): Promise<void> {
    try {
      ...
      // 🔥 使用标准GoogleGenAI SDK，结合我们的Token管理
      this.currentGenAI = new GoogleGenAI({
        apiKey: token
      });

      this.currentToken = token;
      this.moduleLogger.info('Gemini AI service initialized successfully with @google/genai SDK and database-backed token management');
  ...
  private async switchToNextToken(): Promise<boolean> {
    try {
      ...
      if (newToken && newToken !== this.currentToken) {
        ...
        // 🔥 重新创建GoogleGenAI实例使用新Token
        this.currentGenAI = new GoogleGenAI({
          apiKey: newToken
        });

        this.moduleLogger.info('Switched to next available token and recreated GoogleGenAI instance');
        return true;
      }
  ...
  /**
   * 🔥 P0重构：统一配置加载方法
   * 使用统一配置系统加载Agent配置，支持配置转换和缓存
   */
  private async getUnifiedConfiguration(agentType: string, promptName?: string): Promise<UnifiedLLMConfig | null> {
    const cacheKey = `unified:${agentType}:${promptName || 'default'}`;

    // 检查统一配置缓存
    if (this.unifiedConfigCache.has(cacheKey)) {
      return this.unifiedConfigCache.get(cacheKey)!;
    }

    try {
      // 1. 尝试获取现有的AgentPromptData
      const legacyPrompt = await this.getAgentPrompt(agentType, promptName);
      if (!legacyPrompt) {
        this.moduleLogger.warn('No agent prompt found for unified configuration', { agentType, promptName });
        return null;
      }

      // 2. 使用配置转换器转换为统一配置
      const unifiedConfig = this.configConverter.fromAgentPromptData(legacyPrompt);

      // 3. 验证配置完整性
      const validation = this.configConverter.validateConfig(unifiedConfig);
      if (!validation.isValid) {
        this.moduleLogger.error('Invalid unified configuration', {
          agentType,
          promptName,
      ...
      if (validation.warnings.length > 0) {
        this.moduleLogger.warn('Configuration warnings', {
          agentType,
          promptName,
      ...
      // 5. 缓存统一配置
      this.unifiedConfigCache.set(cacheKey, unifiedConfig);

      this.moduleLogger.debug('Unified configuration loaded successfully', {
        agentType,
        promptName,
        modelName: unifiedConfig.model.name,
        configVersion: unifiedConfig.version.version
      });

      return unifiedConfig;
    } catch (error) {
      this.moduleLogger.error('Failed to load unified configuration', { error, agentType, promptName });
      return null;
    }
  }
   ...
   * 🔥 P0重构：清理统一配置缓存
   */
  private clearUnifiedConfigCache(agentType?: string): void {
    if (agentType) {
      // 清理特定agent类型的缓存
      const keysToDelete = Array.from(this.unifiedConfigCache.keys()).filter(key =>
        key.includes(`unified:${agentType}:`)
      );
      keysToDelete.forEach(key => this.unifiedConfigCache.delete(key));
    } else {
      // 清理全部缓存
      this.unifiedConfigCache.clear();
    }
  }

  /**
   * 🔥 P0重构：基于统一配置的Gemini API调用
   * 使用统一配置系统替代传统配置加载方式
   */
  private async callGeminiAPIWithUnifiedConfig(
    prompt: string,
    agentType: string = 'chat_bot',
    promptName: string = 'default_chat',
    traceId?: string,
    userId?: number
  ): Promise<{ response: string; rawResponse: any; usedConfig?: UnifiedLLMConfig; requestBody?: any; modelName: string } | null> {
    const llmCallId = uuidv4();
    const callStartTime = Date.now();

    // 🔥 埋点：LLM调用开始
    if (traceId && this.loggingService) {
      await this.loggingService.logEventStart(traceId, 'llm', 'gemini_api_call_unified', undefined, {
    ...
    try {
      // 1. 获取统一配置
      const unifiedConfig = await this.getUnifiedConfiguration(agentType, promptName);
      if (!unifiedConfig) {
        throw new Error(`No unified configuration found for ${agentType}/${promptName}`);
      }

      // 2. 提取模型和生成配置
      const modelName = unifiedConfig.model.name;
      const generationConfig = unifiedConfig.generation;
      const safetySettings = unifiedConfig.safety;

      this.moduleLogger.debug('Using unified configuration for API call', {
        agentType,
        promptName,
        modelName,
        configVersion: unifiedConfig.version.version
      ...
      // 3. 使用Model-aware Token获取
      const tokenInfo = await this.tokenManager.getTokenForModel(modelName, agentType, promptName);
      if (!tokenInfo) {
        throw new Error(`No available tokens for model ${modelName} and prompt ${agentType}/${promptName}`);
      }

      // 4. 构建系统指令
      const systemContext = unifiedConfig.context.systemInstruction || '';
      ...
      const requestBody = {
        ...
        safetySettings: safetySettings.map(safety => ({
      ...
      const response = await axios.post(
        `${this.baseURL}/${modelName}:generateContent?key=${tokenInfo.token}`,
        requestBody,
        {
          headers: {
            'Content-Type': 'application/json'
          },
          timeout: unifiedConfig.performance.timeout || 30000
        }
      );
      ...
      if (response.data?.candidates?.[0]?.content?.parts?.[0]?.text) {
        ...
        this.moduleLogger.info('Unified config API call successful', {
          modelName,
          agentType,
          promptName,
      ...
      this.moduleLogger.error('Unified config API call failed', {
        error: error.message,
        agentType,
        promptName,
        traceId
      ...
      // 🔥 埋点：LLM调用失败
      if (traceId && this.loggingService) {
        await this.loggingService.logEventEnd(traceId, 'llm', 'gemini_api_call_unified', new Date(callStartTime), undefined, {
          status: 'failed',
  ...
  private async callGeminiAPI(
    ...
    // 🔥 埋点：LLM调用开始
    if (traceId && this.loggingService) {
      await this.loggingService.logEventStart(traceId, 'llm', 'gemini_api_call', undefined, {
        agent_type: agentType,
        prompt_name: promptName,
        llm_call_id: llmCallId
      });
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // 🔥 使用标准GoogleGenAI SDK替代axios调用
        if (!this.currentGenAI) {
        ...
        const config = {
          ...
          topP: modelConfig.topP,
          maxOutputTokens: modelConfig.maxOutputTokens,
          // 🔥 新增：Gemini安全设置 - 解除所有内容限制，使用正确的类型
          safetySettings: [
            {
        ...
        const result = await this.currentGenAI.models.generateContent({
          model: modelName,
          config,
          contents
        });
        ...
        // 添加详细的响应结构日志用于调试 - 适配SDK响应格式
        this.moduleLogger.debug('Raw Gemini SDK response structure', {
        ...
        try {
          ...
          if (!response.candidates) {
            this.moduleLogger.error('API响应缺少candidates字段', { 
              ...
              responseStructure: 'SDK response format' 
            });
        ...
        // 🔥 埋点：LLM调用成功完成
        if (traceId && this.loggingService) {
          await this.loggingService.logInstantEvent(traceId, 'llm', 'gemini_api_completed', undefined, {
            agent_type: agentType,
  ...
  public async generateResponse(
    ...
    try {
      // 🔥 P0重构：根据开关选择配置系统
      const callResult = this.useUnifiedConfig
        ? await this.callGeminiAPIWithUnifiedConfig(userMessage, agentType, promptName, traceId, userId)
        : await this.callGeminiAPI(userMessage, agentType, promptName, traceId, userId);
      ...
      // 兼容两种配置系统的返回值
      const { response, rawResponse, modelName } = callResult;
      const usedPrompt = this.useUnifiedConfig ? null : (callResult as any).usedPrompt;
      const usedConfig = this.useUnifiedConfig ? (callResult as any).usedConfig : null;
  ...
  /**
   * 使用配置调用LLM - 支持所有Gemini高级功能
   */
  public async callWithConfig(
    prompt: string,
    ...
    try {
      ...
      // 2. 确保有可用Token和模型
      await this.ensureTokenForModel(config.modelName);
      if (!this.currentGenAI) {
        throw new Error('No available Gemini AI instance');
      }

      // 3. 构建Gemini API配置
      const geminiConfig = await this.buildGeminiConfig(config);

      // 4. 构建请求内容
      const content = await this.buildContent(prompt, config);
      ...
      const requestParams = {
        ...
        toolConfig: geminiConfig.toolConfig,
      ...
      try {
        result = await this.currentGenAI.models.generateContent(requestParams);
        apiCallTime = Date.now() - apiCallStart;
      } catch (apiError: any) {
        apiCallTime = Date.now() - apiCallStart;

        // 处理Token相关错误
        if (this.isTokenError(apiError)) {
          await this.handleTokenError(apiError);
          throw new Error('Token error occurred, please retry');
        }
      ...
      // 7. 处理响应
      const response = await this.processGeminiResponse(result, config, {
      ...
      if (traceId && this.loggingService) {
        await this.loggingService.logLLMCall({
          ...
          sessionId: '',
          callSequence: 1,
          agentType: 'configurable_llm',
          modelName: config.modelName,
          modelProvider: 'gemini',
          promptTemplate: JSON.stringify(config),
          inputPrompt: prompt,
          ...
          apiCallTimeMs: response.metrics.apiCallTimeMs,
          processingTimeMs: response.metrics.processingTimeMs,
          status: 'SUCCESS',
          errorMessage: '',
          errorCode: '',
          costEstimate: 0,
          tokenUsage: response.metrics,
          userId: userId || 0,
          contextSummary: 'Advanced LLM call with full configuration'
        });
      }
      ...
      if (traceId && this.loggingService) {
        await this.loggingService.logLLMCall({
          ...
          tokenUsage: {},
          userId: userId || 0,
          contextSummary: 'Failed LLM call with configuration'
        });
      }
   ...
   * 验证LLM配置有效性
   */
  public async validateConfig(config: LLMCallConfig): Promise<{
    valid: boolean;
    errors: string[];
  }> {
    const errors: string[] = [];
    ...
    // 2. 检查模型支持的功能
    try {
      const capabilities = await this.getModelCapabilities(config.modelName);

      // 验证思考模式
      if (config.thinkingConfig?.thinkingBudget !== undefined) {
        if (!capabilities.supportsThinking) {
          errors.push(`Model ${config.modelName} does not support thinking mode`);
        } else {
          const budget = config.thinkingConfig.thinkingBudget;
          if (budget !== -1 && budget !== 0 && (budget < 0 || budget > 32768)) {
            errors.push('Thinking budget must be -1 (dynamic), 0 (disabled), or positive integer <= 32768');
          }
        }
      }
      ...
      if (config.tools && config.tools.length > 0) {
        if (!capabilities.supportsFunctionCalling) {
          errors.push(`Model ${config.modelName} does not support function calling`);
        }
        if (config.tools.length > 20) {
          errors.push('Maximum 20 tools are supported');
        }
      }

      // 验证Google搜索
      if (config.googleSearch?.enabled) {
        if (!capabilities.supportsGoogleSearch) {
          errors.push(`Model ${config.modelName} does not support Google Search`);
        }
      }

      // 验证URL上下文
      if (config.urlContext?.enabled) {
        if (!capabilities.supportsUrlContext) {
          errors.push(`Model ${config.modelName} does not support URL context`);
        }
        if (config.urlContext.maxUrls && config.urlContext.maxUrls > 20) {
          errors.push('Maximum 20 URLs are supported');
        }
      }
      ...
      if (config.structuredOutput?.enabled) {
        if (!capabilities.supportsStructuredOutput) {
          errors.push(`Model ${config.modelName} does not support structured output`);
        }
      }

    } catch (error) {
      errors.push(`Failed to verify model capabilities: ${error}`);
    }
    ...
    if (config.generationConfig) {
      ...
      if (gen.topP !== undefined && (gen.topP < 0 || gen.topP > 1)) {
        errors.push('TopP must be between 0 and 1');
      }

      if (gen.topK !== undefined && gen.topK < 1) {
        errors.push('TopK must be a positive integer');
      }

      if (gen.maxOutputTokens !== undefined && gen.maxOutputTokens < 1) {
        errors.push('MaxOutputTokens must be a positive integer');
      }
    }
   ...
   * 获取模型支持的功能
   */
  public async getModelCapabilities(modelName: string): Promise<{
    ...
    // 检查缓存
    if (this.modelCapabilitiesCache.has(modelName)) {
      return this.modelCapabilitiesCache.get(modelName);
    }

    // 基于模型名称确定能力
    const capabilities = this.determineModelCapabilities(modelName);
   ...
   * 确保为指定模型有可用Token
   */
  private async ensureTokenForModel(modelName: string): Promise<void> {
    if (!this.currentGenAI || !this.currentToken) {
      const token = await this.tokenManager.getNextTokenForModel(modelName);
      if (!token) {
        throw new Error(`No available tokens for model: ${modelName}`);
      }

      this.currentToken = token;
      this.currentGenAI = new GoogleGenAI({ apiKey: token });
    }
  }
   ...
   * 构建Gemini API配置
   */
  private async buildGeminiConfig(config: LLMCallConfig): Promise<{
    ...
    // 1. 生成配置 - 基于实际API结构
    if (config.generationConfig || config.thinkingConfig || config.structuredOutput?.enabled) {
      geminiConfig.generationConfig = {};

      if (config.generationConfig) {
        Object.assign(geminiConfig.generationConfig, {
          ...
          responseSchema: config.generationConfig.responseSchema
        });
      }
      ...
      if (config.thinkingConfig) {
        ...
        if (config.thinkingConfig.thinkingBudget !== undefined) {
          thinkingConfig.thinkingBudget = config.thinkingConfig.thinkingBudget;
        }
        if (config.thinkingConfig.includeThoughts !== undefined) {
          thinkingConfig.includeThoughts = config.thinkingConfig.includeThoughts;
        }
        geminiConfig.generationConfig.thinkingConfig = thinkingConfig;
      }

      // 结构化输出
      if (config.structuredOutput?.enabled) {
        if (config.structuredOutput.jsonSchema) {
          geminiConfig.generationConfig.responseMimeType = 'application/json';
          geminiConfig.generationConfig.responseSchema = config.structuredOutput.jsonSchema;
        }
      }
    }
    ...
    if (config.safetySettings) {
      geminiConfig.safetySettings = config.safetySettings.map(setting => ({
    ...
    // 3. 工具配置
    if (config.tools || config.googleSearch?.enabled) {
      geminiConfig.tools = [];
      ...
      if (config.googleSearch?.enabled) {
        geminiConfig.tools.push({
          googleSearch: {
            excludeDomains: config.googleSearch.dynamicThreshold ? [] : undefined
          }
        });
      }
      ...
      if (config.toolConfig?.functionCallingConfig) {
        geminiConfig.toolConfig = {
          functionCallingConfig: {
            mode: config.toolConfig.functionCallingConfig.mode,
   ...
   * 构建请求内容
   */
  private async buildContent(prompt: string, config: LLMCallConfig): Promise<string> {
    // 1. 基础文本内容
    let finalPrompt = prompt;
    if (config.promptPrefix) {
      finalPrompt = config.promptPrefix + '\n\n' + finalPrompt;
    }
  ...
  private async processGeminiResponse(
    ...
    const response: LLMCallResponse = {
      content: '',
      rawResponse: result,
      usedConfig: config,
      metrics
    ...
    if (result.text) {
      ...
      response.content = result.candidates[0].content.parts[0].text;
    }

    // 提取思考过程
    if (config.thinkingConfig?.includeThoughts && result.candidates?.[0]?.content) {
      // 查找思考部分
      const candidate = result.candidates[0];
      if (candidate.content.parts?.some((part: any) => part.thought)) {
        response.thoughts = candidate.content.parts
    ...
    // 提取函数调用
    if (config.tools && result.candidates?.[0]?.content?.parts) {
    ...
    // 提取搜索信息
    if (config.googleSearch?.enabled && result.candidates?.[0]?.groundingMetadata) {
      const metadata = result.candidates[0].groundingMetadata;
      if (metadata.webSearchQueries) {
        response.searchQueries = metadata.webSearchQueries;
      }
      if (metadata.groundingChunks) {
        response.groundingChunks = metadata.groundingChunks.map((chunk: any) => ({
  ...
  /**
   * 从Agent Prompt数据加载完整LLM配置
   */
  public async callWithAgentPrompt(
    prompt: string,
    ...
    try {
      // 1. 获取Agent Prompt配置 (包含高级配置)
      const agentPrompt = await this.getAgentPromptWithAdvancedConfig(agentPromptId);
      if (!agentPrompt) {
        throw new Error(`Agent prompt not found: ${agentPromptId}`);
      }

      // 2. 转换为LLMCallConfig
      const llmConfig = this.convertAgentPromptToLLMConfig(agentPrompt);

      // 3. 调用配置化LLM
      return await this.callWithConfig(prompt, llmConfig, traceId, userId);

    } catch (error: any) {
      this.moduleLogger.error('Failed to call with agent prompt', {
        agentPromptId,
        error: error.message,
        traceId
      });
  ...
  /**
   * 获取Agent Prompt及其高级配置
   */
  private async getAgentPromptWithAdvancedConfig(agentPromptId: string): Promise<AgentPromptData | null> {
    try {
      const query = `
        SELECT
      ...
      const results = await this.database.executeQuery(query, [agentPromptId]);
      ...
      return {
        ...
        system_instructions: JSON.parse(row.system_instructions || '[]'),
        user_prompt_template: row.user_prompt_template,
        context_variables: JSON.parse(row.context_variables || '{}'),
        model_name: row.model_name,
        model_config: JSON.parse(row.model_config || '{}'),
        advanced_config: JSON.parse(row.advanced_config || '{}'),
        config_version: row.config_version,
        last_config_update: row.last_config_update,
      ...
      this.moduleLogger.error('Failed to get agent prompt with advanced config', {
        agentPromptId,
        error: error.message
      });
      throw error;
  ...
  /**
   * 将Agent Prompt配置转换为LLMCallConfig
   */
  private convertAgentPromptToLLMConfig(agentPrompt: AgentPromptData): LLMCallConfig {
    const config: LLMCallConfig = {
      modelName: agentPrompt.model_name || 'gemini-2.5-flash'
    };

    // 系统指令
    if (agentPrompt.system_instructions && agentPrompt.system_instructions.length > 0) {
      config.systemInstruction = agentPrompt.system_instructions.join('\n');
    }

    // 如果有高级配置，使用高级配置
    if (agentPrompt.advanced_config) {
      const advancedConfig = agentPrompt.advanced_config;

      // 生成配置
      if (advancedConfig.generationConfig) {
        config.generationConfig = advancedConfig.generationConfig;
      }

      // 思考配置
      if (advancedConfig.thinkingConfig) {
        config.thinkingConfig = advancedConfig.thinkingConfig;
      }

      // 安全设置
      if (advancedConfig.safetySettings) {
        config.safetySettings = advancedConfig.safetySettings;
      }

      // 工具配置 - 🆕 支持预定义工具
      if (advancedConfig.toolsConfig && advancedConfig.toolsConfig.enabled) {
        const toolsConfig = advancedConfig.toolsConfig;

        // 获取工具声明
        if (toolsConfig.selectedTools && toolsConfig.selectedTools.length > 0) {
          config.tools = ToolManager.getToolDeclarations(toolsConfig.selectedTools);
        }

        // 工具调用配置
        config.toolConfig = {
          functionCallingConfig: {
            mode: toolsConfig.mode || 'AUTO',
            allowedFunctionNames: toolsConfig.allowedTools || toolsConfig.selectedTools
          }
        };
      }

      // Google搜索配置
      if (advancedConfig.googleSearchConfig) {
        config.googleSearch = advancedConfig.googleSearchConfig;
      }

      // URL上下文配置
      if (advancedConfig.urlContextConfig) {
        config.urlContext = advancedConfig.urlContextConfig;
      }

      // 结构化输出配置
      if (advancedConfig.structuredOutputConfig) {
        config.structuredOutput = advancedConfig.structuredOutputConfig;
      }

      // 提示词配置
      if (advancedConfig.promptConfig) {
        config.promptPrefix = advancedConfig.promptConfig.promptPrefix;
        config.promptSuffix = advancedConfig.promptConfig.promptSuffix;
      }
    } else {
      // 兼容旧的model_config
      if (agentPrompt.model_config) {
        config.generationConfig = {
          temperature: agentPrompt.model_config.temperature,
          topP: agentPrompt.model_config.topP,
          topK: agentPrompt.model_config.topK,
          maxOutputTokens: agentPrompt.model_config.maxOutputTokens
        };
      }
    }
   ...
   * 更新Agent Prompt的高级配置
   */
  public async updateAgentAdvancedConfig(
    agentPromptId: string,
    ...
    try {
      // 验证工具配置
      if (advancedConfig.toolsConfig && advancedConfig.toolsConfig.selectedTools) {
        const validation = ToolManager.validateToolConfig(advancedConfig.toolsConfig.selectedTools);
        if (!validation.valid) {
      ...
      // 清除缓存
      this.advancedConfigCache.delete(agentPromptId);
      this.promptCache.delete(agentPromptId);

      this.moduleLogger.info('Agent advanced config updated', {
        agentPromptId,
        updatedBy,
        configKeys: Object.keys(advancedConfig)
      });

      return true;

    } catch (error: any) {
      this.moduleLogger.error('Failed to update agent advanced config', {
        agentPromptId,
        error: error.message
      });
      throw error;
   ...
   * 获取可用的预定义工具列表
   */
  public getAvailableTools(): Array<{key: string; name: string; description: string; category: string}> {
   ...
   * 根据Agent类型推荐工具
   */
  public getRecommendedToolsForAgent(agentType: string): string[] {
    return ToolManager.getRecommendedTools(agentType);
  }

  /**
   * 确定模型能力
   */
  private determineModelCapabilities(modelName: string): any {
    // 基于模型名称返回能力配置
    ...
    if (modelName.includes('1.5')) {
      return {
        ...baseCapabilities,
        supportsThinking: false, // 1.5模型不支持思考模式
        maxInputTokens: 1048576,
  ...
  private async handleTokenError(error: any): Promise<void> {
    if (this.currentToken) {
      await this.tokenManager.reportError(this.currentToken, error.message);
  ...
  /**
   * 启用统一配置系统
   * @param enabled 是否启用统一配置系统
   */
  public enableUnifiedConfiguration(enabled: boolean = true): void {
    this.useUnifiedConfig = enabled;
    this.moduleLogger.info('Unified configuration system', {
      enabled: enabled ? 'ENABLED' : 'DISABLED'
    });

    if (enabled) {
      this.moduleLogger.info('🔥 AI Service now using UNIFIED configuration system');
    } else {
      this.moduleLogger.info('AI Service using LEGACY configuration system');
    }
  }
   ...
   * 获取当前配置系统状态
   */
  public isUnifiedConfigurationEnabled(): boolean {
   ...
   * 为常用的Agent类型预加载配置
   */
  public async preloadUnifiedConfigurations(agentTypes: string[] = ['chat_bot', 'decision_engine', 'persona_engine']): Promise<void> {
    this.moduleLogger.info('Preloading unified configurations', { agentTypes });

    const preloadPromises = agentTypes.map(async (agentType) => {
      try {
        const config = await this.getUnifiedConfiguration(agentType);
        if (config) {
          this.moduleLogger.debug('Preloaded unified configuration', { agentType, configId: config.id });
        }
      } catch (error) {
        this.moduleLogger.warn('Failed to preload unified configuration', { agentType, error });
      }
    });

    await Promise.allSettled(preloadPromises);
    this.moduleLogger.info('Unified configuration preloading completed');
  }

  /**
   * 清理配置缓存
   * @param type 清理类型：'legacy' | 'unified' | 'all'
   */
  public clearConfigurationCache(type: 'legacy' | 'unified' | 'all' = 'all'): void {
    if (type === 'legacy' || type === 'all') {
      this.promptCache.clear();
      this.advancedConfigCache.clear();
      this.moduleLogger.debug('Legacy configuration cache cleared');
    }

    if (type === 'unified' || type === 'all') {
      this.unifiedConfigCache.clear();
      this.moduleLogger.debug('Unified configuration cache cleared');
    }

    this.moduleLogger.info('Configuration cache cleared', { type });
  }
  ...
  public getConfigurationStats(): {
    system: 'legacy' | 'unified';
    cacheStats: {
      legacy: { prompts: number; advancedConfigs: number };
      unified: number;
    };
    capabilities: string[];
  } {
    return {
      system: this.useUnifiedConfig ? 'unified' : 'legacy',
      cacheStats: {
        legacy: {
          prompts: this.promptCache.size,
          advancedConfigs: this.advancedConfigCache.size
        },
        unified: this.unifiedConfigCache.size
      },
      capabilities: [
        'model-aware-tokens',
        ...
        'cache-management',
        'version-control',
        this.useUnifiedConfig ? 'unified-config' : 'legacy-config'
      ]
    };
  }
}
