...
def test_identity_when_live_equals_declared():
    ...
    _check("center", cc.map_point(640, 400, 1280, 800), (640, 400))
    _check("origin", cc.map_point(0, 0, 1280, 800), (0, 0))


def test_downscale_live_smaller_than_declared():
    # live 640x400, declared 1280x800 -> half on each axis
    _check("full-edge", cc.map_point(1280, 800, 640, 400), (639, 399))  # full extent, clamped to w-1,h-1
    _check("mid", cc.map_point(640, 400, 640, 400), (320, 200))


def test_upscale_live_larger_than_declared():
    # live 2560x1600, declared 1280x800 -> x2 on each axis
    _check("double", cc.map_point(640, 400, 2560, 1600), (1280, 800))


def test_clamp_out_of_bounds():
    # model returns a coord past the edge -> clamped into the page
    _check("past-right", cc.map_point(2000, 400, 1280, 800), (1279, 400))
    _check("past-bottom", cc.map_point(640, 2000, 1280, 800), (640, 799))
    _check("negative", cc.map_point(-5, -5, 1280, 800), (0, 0))


def test_rounding():
    # 1281-wide live, declared 1280 -> 1.000781 scale, rounds to nearest int
    _check("round", cc.map_point(100, 100, 1281, 801), (100, 100))
    _check("round2", cc.map_point(1000, 500, 1281, 801), (1001, 501))
...
def test_map_region_normalizes_and_maps():
    ...
    got = cc.map_region([100, 100, 300, 200], 1280, 800)
    _check("region", got, (100, 100, 300, 200))
    # reversed corners get normalized
    got2 = cc.map_region([300, 200, 100, 100], 640, 400)
