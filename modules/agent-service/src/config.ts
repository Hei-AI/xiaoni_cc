...
export const agentConfig = {
  ...
  selfActionStartupGraceMs: Math.max(0, Number.parseInt(process.env.SELF_ACTION_STARTUP_GRACE_MS || String(10 * 60 * 1000), 10)),
  selfActionDailyBudget: Math.max(0, Number.parseInt(process.env.SELF_ACTION_DAILY_BUDGET || '3', 10)),
  selfActionHourlyBudget: Math.max(0, Number.parseInt(process.env.SELF_ACTION_HOURLY_BUDGET || '1', 10)),
  selfActionMaxConsecutiveWithoutUser: Math.max(0, Number.parseInt(process.env.SELF_ACTION_MAX_CONSECUTIVE_WITHOUT_USER || '1', 10)),
  selfActionModelName: process.env.SELF_ACTION_MODEL_NAME || process.env.AI_MODEL_NAME || 'gpt-5.4-mini',
