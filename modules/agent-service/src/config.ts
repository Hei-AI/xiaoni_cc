...
export const agentConfig = {
  ...
  preReplyMemoryReasonerEnabled: process.env.AGENT_PRE_REPLY_MEMORY_REASONER_ENABLED === 'true',
  preReplyMemoryReasonerModelName: process.env.AGENT_PRE_REPLY_MEMORY_REASONER_MODEL || 'gpt-5.4',
  presentSelfReconstructionEnabled: process.env.AGENT_PRESENT_SELF_RECONSTRUCTION_ENABLED !== 'false',
  ...
  systemPrompt: process.env.AGENT_LOOP_SYSTEM_PROMPT || [
    ...
    'Available tools are build_memory_rag_context, retrieve_memory_hints, reply_in_private, speak_in_group, and stay_silent.',
    'When the live thread is not enough and historical trajectory matters, call build_memory_rag_context first.',
    'Call retrieve_memory_hints only after build_memory_rag_context when you still need tighter relationship or tone guidance.',
    'Memory hints shape the reply. They do not replace long-context history and they do not automatically grant speaking permission.',
    ...
    'If you use any read tool, finish the run with exactly one speaking decision: reply_in_private, speak_in_group, or stay_silent.',
