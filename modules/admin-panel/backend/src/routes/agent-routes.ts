...
export function createAgentRoutes(database: DatabaseManager, logger: winston.Logger) {
  ...
  // 获取特定Agent类型详情
  router.get('/agent-types/:id', async (req, res) => {
    try {
      ...
      const agent = await database.executeQuery(`
        SELECT *
        FROM agent_prompts
        WHERE id = ?
      `, [agentId]);
      ...
      // 获取该Agent的使用历史
      const usageHistory = await database.executeQuery(`
        SELECT
        ...
        LIMIT 30
      `, [(agent[0] as any).model_name]);

      const agentData = agent[0] as any;
      let allowedTokenIds: number[] = [];
      try {
        allowedTokenIds = agentData.allowed_token_ids ? JSON.parse(agentData.allowed_token_ids) : [];
      } catch (e) {
        logger.warn(`Invalid allowed_token_ids JSON for agent ${agentId}`, { allowed_token_ids: agentData.allowed_token_ids });
      }
      ...
      logger.error('Failed to fetch agent details', { error, agentId: req.params.id });
      res.status(500).json({
        success: false,
