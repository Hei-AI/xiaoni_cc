...
export function createAgentRuntimeRoutes(database: DatabaseManager, logger: winston.Logger) {
  ...
  router.get('/xiaoni/passive-recall/recall', async (req, res) => {
    try {
      ...
      const centeredStandoutMargin = Number.isFinite(Number(process.env.XIAONI_RECALL_STANDOUT_MARGIN)) ? Number(process.env.XIAONI_RECALL_STANDOUT_MARGIN) : 0.25;
      const centeredNearDupSuppress = Number.isFinite(Number(process.env.XIAONI_RECALL_NEARDUP_SUPPRESS)) ? Number(process.env.XIAONI_RECALL_NEARDUP_SUPPRESS) : 0.95;
      const centeredBand = meanVector ? { floor: taskLocked ? centeredTaskFloor : centeredFloor, ceiling: centeredCeiling, standoutMargin: centeredStandoutMargin, nearDupSuppress: centeredNearDupSuppress } : {};
  ...
  router.get('/agent-runtime/deep-dives', async (req, res) => {
    try {
      ...
      res.json({
        ...
        data: {
          ...
          stats: {
            ...
            byPhase: history.reduce<Record<string, number>>((acc, dive) => {
              acc[dive.phase] = (acc[dive.phase] ?? 0) + 1;
              return acc;
            }, {}),
            // 「集中在 1」就是逃生舱信号
      ...
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to load Xiaoni deep dives',
        timestamp: new Date().toISOString()
      });
    }
  });
  ...
  router.get('/agent-runtime/failure-reviews', async (req, res) => {
    try {
      ...
      // 连接键是 fork_run_id,不是 deep_dive_id。一次复核 = 一个 fork_run_id,轮数硬上界
      // FAILURE_REVIEW_FORK_MAX_TURNS = 32,所以 limit 是**算出来的**不是猜的。
      // (按深挖归组拿不到上界:同一次深挖可以反复 blocked,每次都是独立一跑。)
      ...
      res.json({
        ...
        data: rows.map((row) => ({
          ...
          // 键名必须逐字对上写端(agent-loop-service 的 timeline metadata 与 baseForkMetadata,
          // 两处现已统一为 deep_dive_*)。此处历史上恒为 null 过一次 —— 读写两端各写各的,
          // 而当时的用例是拿读取侧的形状手工造数据验的,绿的是同义反复。
          diveId: row.metadata?.deep_dive_id ?? null,
          question: row.metadata?.deep_dive_question ?? null,
