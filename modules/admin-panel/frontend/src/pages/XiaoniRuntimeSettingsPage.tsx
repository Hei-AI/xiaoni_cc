        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-2">
              ...
              <div className="text-xs text-muted-foreground">
                <strong>没有日额。</strong>联想不是配额制的——人不会「今天已经想起过 10 件事，后面就不想了」。该不该冒由精排 Agent（Sonnet 4.6）一条一条判，它可以说「一条都不值得」，而且多数时候就该这么说；它不在场时退回最小间隔，兜的是「判断力缺席」不是「今天够了」。同一段记忆永远只投一次（dedupe 锚在记忆本身的 ref 上）。投递跟着事件走：她消费一条 QQ 消息、或自己每次落地，那次召回到的东西当场交精排 Agent，不再定时、不设时段——她睡着时消息不会被消费，自然不投。
              </div>
            </div>
            <div className="flex items-center gap-3">
              {mutation.isPending ? <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" /> : null}
