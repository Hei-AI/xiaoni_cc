...
function XiaoniUsageObservatory({
  ...
  onSelectWindow: (startTime: Date, endTime: Date) => void;
  ...
  const domainStartMs = chartPoints[0]?.timestampMs || parseTimestampMs(timeline?.window.startTime) || Date.now() - TIME_RANGE_DURATION_MS['24h']!;
  const domainEndMs = chartPoints[chartPoints.length - 1]?.timestampMs || parseTimestampMs(timeline?.window.endTime) || Date.now();
  ...
  const fullEndMs = parseTimestampMs(timeline?.dataBounds.lastAt) || chartDomain[1];
  const windowStartMs = parseTimestampMs(timeline?.window.startTime) || chartDomain[0];
  const windowEndMs = parseTimestampMs(timeline?.window.endTime) || chartDomain[1];
  ...
  const commitWheelWindow = React.useCallback((nextStart: number, nextEnd: number) => {
    ...
    wheelCommitTimerRef.current = window.setTimeout(() => {
      ...
      if (pending) {
        onSelectWindow(new Date(pending[0]), new Date(pending[1]));
      }
    }, 260);
  }, [onSelectWindow]);
  ...
  const handleMouseDown = React.useCallback((state: unknown) => {
    ...
    setSelectionStartMs(timestampMs);
    setSelectionEndMs(timestampMs);
  }, []);
  ...
  const handleMouseUp = React.useCallback(() => {
    ...
    onSelectWindow(new Date(startMs), new Date(endMs));
  }, [clearSelection, onSelectWindow, selectionEndMs, selectionStartMs]);
  ...
  const handleWheel = React.useCallback((event: React.WheelEvent<HTMLDivElement>) => {
    ...
    commitWheelWindow(nextStart, nextEnd);
  }, [commitWheelWindow, fullEndMs, fullStartMs, timeline, visibleEndMs, visibleStartMs]);
...
function TimelineEvent({ item, isLatest, isFocused }: { item: XiaoniActivityFeedItem; isLatest: boolean; isFocused: boolean }) {
          ...
          {traceTarget?.internalExecutionLeaseId && (item.eventId || item.id) ? (
            <Button
              ...
              onClick={() => {
                ...
                navigate(`/xiaoni/action-stream/events/${encodeURIComponent(itemEventId)}/trace${focusSpanId ? `?spanId=${encodeURIComponent(focusSpanId)}` : ''}`);
              }}
            >
...
export const XiaoniActivityPage: React.FC = () => {
  ...
  const handleUsageWindowSelect = React.useCallback((nextStartTime: Date, nextEndTime: Date) => {
    updateSearchParam((nextParams) => {
      nextParams.set('range', 'custom');
