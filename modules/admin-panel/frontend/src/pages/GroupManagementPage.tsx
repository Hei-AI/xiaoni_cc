...
const updateGroup = async (
  groupId: number,
  data: { is_enabled?: number; group_name?: string | null; welcome_message?: string | null }
) => {
...
export const GroupManagementPage: React.FC = () => {
  ...
  const groupsQueryKey = ['groups', page, search] as const;
  ...
  const renderToggleControl = (group: GroupChat, field: GroupToggleField, label: string) => {
    ...
    const disabled = loadingStates[loadingKey] || false;
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>输入指定群号后，默认会创建为 IM 入口开启。</DialogDescription>
          </DialogHeader>
                ...
                <TableBody>
                  {rows.map((group) => (
                    <TableRow key={group.group_id}>
                      ...
                      <TableCell>
                        <div className="flex min-w-[96px] items-center gap-4">
                          <div className="flex items-center gap-2">
