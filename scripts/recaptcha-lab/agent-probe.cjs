...
(async () => {
  ...
  const result = await service.runSherlockFork({
    diveId: '', forkRunId: id,
    question: `请实际操作浏览器，测试用户自建的验证码页面 ${url}：打开新标签页，完成 Google reCAPTCHA 复选框及页面出现的挑战，点击提交验证，读取最终服务端验证结果并截图。必须基于页面实际结果报告；注明页面是官方测试模式还是真实模式，是否实际出现图片挑战。不要修改网页或响应，不要直接注入 token，不要伪造验证成功。若无法完成，报告具体阻塞。`,
