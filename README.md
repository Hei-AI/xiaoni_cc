...
- 小腻主 prompt 只有一套，维护在 `docs/xiaoni_prompt/`；`agent-service` 读取 prompt 文件时按 mtime/size 刷新缓存，compose 下通过只读 bind mount 可直接改 prompt 文件并在后续新 queue message / 新 runtime 模板追加时生效。同一条 queue message 的 continuous loop 内，`system prompt` 和稳定 developer 前缀只组装一次。群/私聊不再绑定不同 prompt，DB prompt 表不再是小腻运行时来源。
