...
const remote_claude_service_1 = __importDefault(require("./services/remote-claude-service"));
...
class QQBot {
    constructor() {
        ...
        this.remoteClaudeService = new remote_claude_service_1.default(this.database);
        this.sessionManager = new session_manager_1.SessionManager(this.database);
        this.httpServer = new http_server_1.default(config_1.config.http_server, {
    ...
    async start() {
        try {
            ...
            // 更新机器人状态
            await this.database.updateBotStatus(config_1.config.ai.bot_qq_number.toString(), 'online', this.websocketClient.isConnected(), true);
    ...
    async handleRequirement(userId, message, intent, originalMessage, sessionId) {
        ...
        try {
            ...
            await this.websocketClient.sendPrivateMessage(userId, `❌ 需求处理失败: ${requirementId}\n\n错误信息: ${error instanceof Error ? error.message : '未知错误'}`);
        }
    }
    /**
     * 异步处理需求逻辑
     */
    async processRequirementAsync(requirementData, userId) {
        const { id: requirementId, message } = requirementData;
        try {
            // 检查Remote Claude会话状态
            const sessionExists = await this.remoteClaudeService.checkRemoteSession();
            if (!sessionExists) {
                await this.websocketClient.sendPrivateMessage(userId, `❌ 需求 ${requirementId} 处理失败\n\n错误：Claude Code远程会话未启动\n请联系管理员运行：./scripts/setup_remote_claude.sh`);
                return;
            }
            // 发送开始处理通知
            await this.websocketClient.sendPrivateMessage(userId, `🚀 需求 ${requirementId} 开始处理\n💡 正在通过Claude Code进行自动化开发...`);
            // 实际处理需求
            ...
            await this.websocketClient.sendPrivateMessage(userId, `❌ 需求 ${requirementId} 处理异常\n\n错误: ${error instanceof Error ? error.message : '未知错误'}`);
        }
    }
    /**
     * 格式化处理时间
    ...
    async handleAIConversation(userId, message, originalMessage, sessionId) {
        ...
        try {
            ...
            if (originalMessage.message_type === 'group' && originalMessage.group_id) {
                if (shouldSendReply && originalMessage.message_id) {
                    ...
                    await this.websocketClient.sendGroupMessage(originalMessage.group_id, conversation.ai_response);
                }
            }
            else {
                if (shouldSendReply && originalMessage.message_id) {
                    ...
                    await this.websocketClient.sendPrivateMessage(userId, conversation.ai_response);
                }
            }
    ...
    async sendStartupNotification() {
        try {
            ...
            const authorizedUserId = config_1.config.ai.authorized_user_id;
...
🤖 机器人信息:
• QQ号: ${config_1.config.ai.bot_qq_number}
...
您现在可以开始与我对话了！`;
    ...
    async stop() {
        try {
            ...
            // 更新机器人状态
            await this.database.updateBotStatus(config_1.config.ai.bot_qq_number.toString(), 'offline', false, false);
            // 停止各个服务
...
async function main() {
    ...
    try {
        ...
        logger_1.logger.error('main', 'Failed to start application', { error });
        process.exit(1);
    }
}
// 未捕获异常处理
...
process.on('unhandledRejection', (reason, promise) => {
    logger_1.logger.error('main', 'Unhandled Rejection', {
