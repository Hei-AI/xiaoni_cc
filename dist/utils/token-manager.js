...
exports.resetTokenManager = resetTokenManager;
...
class TokenManager {
    constructor(database) {
        this.moduleLogger = logger_1.logger.createModuleLogger('token-manager');
        this.database = database || (0, database_1.getDatabaseManager)(config_1.config.database);
        this.initialize();
    }
    /**
     * 初始化Token管理器
     ...
     * 加载健康检查配置
     */
    async loadHealthConfig() {
        try {
            const configs = await this.database.executeQuery('SELECT * FROM api_token_health_config WHERE enabled = TRUE ORDER BY id DESC LIMIT 1');
    ...
    async getNextToken() {
        try {
            ...
            const availableTokens = await this.database.executeQuery(`
     ...
     */
    async reportSuccess(token, responseTimeMs, geminiUsage) {
        try {
            // 重置错误计数，更新健康状态
            await this.database.executeUpdate(`
        UPDATE api_tokens SET 
     ...
     * @param responseTimeMs 响应时间（毫秒）
     */
    async reportError(token, error, responseTimeMs) {
        try {
    ...
    async getStats() {
        ...
        FROM api_tokens
      `);
            const tokens = await this.database.executeQuery(`
        SELECT 
    ...
    async getTokenByValue(token) {
        try {
            const results = await this.database.executeQuery('SELECT * FROM api_tokens WHERE token = ? LIMIT 1', [token]);
    ...
    async performHealthCheck() {
        try {
            const tokens = await this.database.executeQuery('SELECT * FROM api_tokens WHERE is_active = TRUE');
     ...
     * 检查单个Token健康状态
     */
    async checkTokenHealth(tokenData) {
        const startTime = Date.now();
        let isHealthy = false;
        let errorMessage = '';
        try {
            // 使用简单的API调用测试Token有效性
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent`, {
                contents: [{
                        parts: [{
    ...
    `, [isHealthy, isHealthy, errorMessage, isHealthy, tokenData.id]);
        // 记录健康检查日志
        await this.logTokenAction(tokenData.id, 'health_check', isHealthy ? 'success' : 'error', isHealthy ? undefined : errorMessage, responseTime);
        if (!isHealthy) {
...
function getTokenManager(database) {
