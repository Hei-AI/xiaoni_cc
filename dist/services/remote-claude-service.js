...
exports.RemoteClaudeService = void 0;
const child_process_1 = require("child_process");
const logger_1 = require("../utils/logger");
class RemoteClaudeService {
    constructor(database) {
        this.moduleLogger = logger_1.logger.createModuleLogger('remote-claude-service');
        this.sessionName = 'claude_remote';
        this.scriptPath = '/home/liahua/IdeaProject/qq_bot/scripts/remote_claude_command.sh';
    ...
    async checkRemoteSession() {
        try {
            const process = (0, child_process_1.spawn)('tmux', ['has-session', '-t', this.sessionName]);
     ...
     * 处理需求 - 核心方法
     */
    async processRequirement(requirementData) {
     ...
     */
    async executeClaudeCodeCommand(command) {
        return new Promise((resolve, reject) => {
            this.moduleLogger.debug('Executing Claude Code command', {
                command: command.substring(0, 100) + (command.length > 100 ? '...' : '')
            });
            // 使用remote_claude_command.sh脚本执行命令
            const process = (0, child_process_1.spawn)('bash', [this.scriptPath, command], {
     ...
     * 捕获Claude Code会话的输出
     */
    async captureClaudeCodeOutput() {
        return new Promise((resolve, reject) => {
            // 使用tmux capture-pane获取会话输出
            const process = (0, child_process_1.spawn)('tmux', [
                'capture-pane',
                '-t', this.sessionName,
                '-p'
            ]);
     ...
     * 清理tmux输出，提取有用信息
     */
    cleanTmuxOutput(rawOutput) {
        const lines = rawOutput.split('\n');
        // 寻找Claude Code相关的输出行
        const claudeOutputStart = lines.findIndex(line => line.includes('claude -p') ||
     ...
     */
    async cleanupStaleRequirements() {
        try {
            const staleThreshold = new Date(Date.now() - 60 * 60 * 1000); // 1小时前
            const staleRequirements = await this.database.executeQuery(`SELECT * FROM requirements 
         WHERE status = 'processing' 
         AND processing_start_time < ?`, [staleThreshold]);
            let cleanupCount = 0;
...
exports.RemoteClaudeService = RemoteClaudeService;
exports.default = RemoteClaudeService;
//# sourceMappingURL=remote-claude-service.js.map