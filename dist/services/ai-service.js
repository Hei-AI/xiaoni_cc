...
class AIService {
    constructor(config, database) {
        ...
        this.tokenManager = (0, token_manager_1.getTokenManager)(this.database);
        // 异步初始化
    ...
    async getAgentPromptFromDatabase(agentType, promptName, cacheKey) {
        // 从数据库获取
        try {
    ...
    async callGeminiAPI(prompt, agentType = 'chat_bot', promptName) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const result = await axios_1.default.post(`${this.baseURL}/${this.config.model_name}:generateContent`, requestBody, {
                    headers: {
                ...
                // 成功时报告Token使用成功
                if (this.currentToken) {
                    await this.tokenManager.reportSuccess(this.currentToken, responseTime, response.usageMetadata);
                }
    ...
    async generateResponse(userMessage, userId, agentType = 'chat_bot', promptName) {
        ...
        try {
            ...
            // 报告当前Token错误
            if (this.currentToken) {
                await this.tokenManager.reportError(this.currentToken, error instanceof Error ? error.message : 'Unknown error');
            }
    ...
    async analyzeIntent(message, userId) {
        try {
            ...
            // 报告Token错误
            if (this.currentToken) {
                await this.tokenManager.reportError(this.currentToken, error instanceof Error ? error.message : 'Intent analysis failed');
            }
