...
const HELP_TASK_TYPE = 'xiaoni_help';
const HELP_PROCESS_ID = randomUUID();
...
function createXiaoniHelpPersistence({ getPrismaClient }) {
  async function beginXiaoniHelp(input, config = {}) {
    ...
    if (!input.helpId) {
      await prisma.agentTask.upsert({
        where: { id }, update: {},
        create: {
          id, task_type: HELP_TASK_TYPE, status: 'help_ready', session_key: 'xiaoni:global', chat_type: 'direct',
    ...
    const interrupted = row.status === 'help_running' && !row.claimed_by?.startsWith(`${HELP_PROCESS_ID}:`);
    if (row.status === 'help_running' && !interrupted) return { ok: false, reason: 'help_running', task: row };
    const claim = `${HELP_PROCESS_ID}:${randomUUID()}`;
  ...
  async function finishXiaoniHelp(input, config = {}) {
    ...
    const history = Array.isArray(row.result_json?.history) ? row.result_json.history : [];
    const updated = await prisma.agentTask.updateMany({
      where: { id: row.id, task_type: HELP_TASK_TYPE, claimed_by: input.claim, status: { in: ['help_running', 'help_human_sending'] } },
      data: {
        status: input.status,
