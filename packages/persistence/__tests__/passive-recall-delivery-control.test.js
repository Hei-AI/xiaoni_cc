...
test('schema 自愈:开关列带默认值 FALSE,老库靠 ADD COLUMN IF NOT EXISTS 补上', async () => {
  ...
  // 没有日额列 —— 联想不是配额制的,量由判官一条一条判。
  assert.doesNotMatch(ddl, /passive_recall_delivery_daily_cap/);
});
...
test('没有行 → 投递默认关着(不给一个「忘了关」的世界线)', async () => {
  ...
  assert.equal(control.passiveRecallDeliveryDailyCap, undefined, '日额这个概念已经没有了');
});

test('读库:开关被投影出来', async () => {
  const { persistence } = createPersistence([[{
    identity_key: 'xiaoni',
    enabled: true,
...
test('写库:开关进 SQL', async () => {
  ...
  assert.doesNotMatch(statement, /passive_recall_delivery_daily_cap/);
...
test('没提到这个字段的 patch 不会把它冲掉(CASE 走 ELSE 保持原值)', async () => {
