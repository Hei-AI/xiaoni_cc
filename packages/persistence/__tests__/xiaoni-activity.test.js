...
test('Xiaoni action stream folds visible group delivery facts into one speaking card', async () => {
  const persistence = createPersistence({
    llmRequestSliceRows: [{
      id: '991',
      sliceId: 'slice-send',
      ...
      traceId: 'trace-send',
      runId: 'run-send',
      createdAt: '2026-06-24T08:05:20.000Z',
      tokenUsage: {
        input_tokens: 110000,
        input_tokens_details: { cached_tokens: 108400 },
        output_tokens: 166
      }
    }],
    agentStackRows: [{
      id: '981',
      eventId: 'stack:trace-send:tool-call:call_send',
      identityKey: 'xiaoni',
      stackIndex: 50,
      itemKind: 'function_call',
      role: 'assistant',
      toolCallId: 'call_send',
      llmRequestSliceId: 'slice-send',
      content: {
        type: 'function_call',
        call_id: 'call_send',
        name: 'send_in_group',
        arguments: '{"group_id":253631878,"message":"那个甜瓜像一颗长了网纹的行星。"}'
      },
      traceId: 'trace-send',
      runId: 'run-send',
      createdAt: '2026-06-24T08:05:24.219Z'
    }, {
      id: '982',
      eventId: 'stack:trace-send:tool-output:call_send',
      identityKey: 'xiaoni',
      stackIndex: 51,
      itemKind: 'function_call_output',
      role: 'tool',
      ...
      content: {
        ...
        output: {
          ...
          sent_messages: ['那个甜瓜像一颗长了网纹的行星。']
        }
      },
      traceId: 'trace-send',
      runId: 'run-send',
      createdAt: '2026-06-24T08:05:24.600Z'
    }],
    agentStackToolRows: [{
      id: '972',
      executionId: 'tool:run-send:call_send',
      identityKey: 'xiaoni',
      llmRequestSliceId: 'slice-send',
      llmCallId: 'llm-send',
      traceId: 'trace-send',
      ...
      status: 'completed',
      sideEffect: true,
      startedAt: '2026-06-24T08:05:24.309Z',
      completedAt: '2026-06-24T08:05:24.630Z'
    }],
    lifeEvents: [{
      id: 24427,
      identity_key: 'xiaoni',
      event_kind: 'visible_delivery_committed',
      occurred_at: new Date('2026-06-24T08:05:24.547Z'),
      actor_type: 'xiaoni',
      actor_id: 'xiaoni',
      ...
      identity_key: 'xiaoni',
      event_kind: 'send_in_group',
      occurred_at: new Date('2026-06-24T08:05:24.553Z'),
      surface: 'qq',
      chat_type: 'group',
      session_key: 'qq:group:253631878',
      peer_id: '253631878',
      ...
      trace_id: 'trace-send',
      payload: {
        tool_name: 'send_in_group',
        sent_messages: ['那个甜瓜像一颗长了网纹的行星。']
      }
    }, {
      id: 24429,
      identity_key: 'xiaoni',
      event_kind: 'qq_self_message',
      occurred_at: new Date('2026-06-24T08:05:24.553Z'),
      surface: 'qq',
      chat_type: 'group',
      session_key: 'qq:group:253631878',
      ...
      payload: {
        tool_name: 'send_in_group',
        index: 0,
        content: '那个甜瓜像一颗长了网纹的行星。'
      }
    }]
  });

  const stream = await persistence.getXiaoniActionStream({ limit: 10 });
  const relatedItems = stream.items.filter((item) => item.runId === 'run-send' || item.traceId === 'trace-send');
  const visibleDeliveryItems = relatedItems.filter((item) => item.eventKind === 'visible_delivery_committed');

  assert.deepEqual(visibleDeliveryItems.map((item) => item.id), ['life:24428']);
  assert.equal(visibleDeliveryItems[0].source, 'life_event');
  assert.equal(visibleDeliveryItems[0].kind, 'send_in_group');
  assert.equal(visibleDeliveryItems[0].title, '在群里发言');
  assert.equal(visibleDeliveryItems[0].body, '那个甜瓜像一颗长了网纹的行星。');
});

test('Xiaoni action stream keeps direct self messages when no aggregate delivery card exists', async () => {
  const persistence = createPersistence({
    lifeEvents: [{
      id: 24501,
      identity_key: 'xiaoni',
      event_kind: 'qq_self_message',
      occurred_at: new Date('2026-06-24T08:07:24.553Z'),
      surface: 'qq',
      chat_type: 'direct',
      session_key: 'qq:direct:1129974489:123',
      peer_id: '123',
      actor_type: 'xiaoni',
      actor_id: '1129974489',
      ...
      identity_key: 'xiaoni',
      event_kind: 'qq_self_message',
      occurred_at: new Date('2026-06-24T08:07:24.650Z'),
      surface: 'qq',
      chat_type: 'direct',
      session_key: 'qq:direct:1129974489:123',
      peer_id: '123',
  ...
  assert.deepEqual(stream.items.map((item) => item.id), ['life:24502']);
  assert.equal(stream.items[0].eventKind, 'visible_delivery_committed');
  assert.equal(stream.items[0].body, '私聊发出去了\n第二条也发出去了');
  assert.equal(stream.items[0].metadata.foldedMessageCount, 2);
  assert.deepEqual(stream.items[0].metadata.foldedItemIds, ['life:24501', 'life:24502']);
});

test('Xiaoni action stream folds one message-reading batch into one seen card', async () => {
  const persistence = createPersistence({
    lifeEvents: [{
      id: 24601,
      identity_key: 'xiaoni',
      event_kind: 'surface_visit',
      occurred_at: new Date('2026-06-24T08:09:24.000Z'),
      surface: 'qq',
      chat_type: 'group',
      session_key: 'qq:group:253631878',
      peer_id: '253631878',
      ...
      occurred_at: new Date('2026-06-24T08:09:24.100Z'),
      surface: 'qq',
      chat_type: 'group',
      session_key: 'qq:group:253631878',
      peer_id: '253631878',
      ...
      identity_key: 'xiaoni',
      event_kind: 'qq_message_seen',
      occurred_at: new Date('2026-06-24T08:09:24.200Z'),
      surface: 'qq',
      chat_type: 'group',
      session_key: 'qq:group:253631878',
      peer_id: '253631878',
  ...
  assert.deepEqual(stream.items.map((item) => item.id), ['life:24603']);
  assert.equal(stream.items[0].eventKind, 'qq_message_seen');
  assert.equal(stream.items[0].title, '看到 2 条消息');
  assert.equal(stream.items[0].body, '第一条\n第二条');
  assert.equal(stream.items[0].metadata.foldedSeenMessageCount, 2);
});
