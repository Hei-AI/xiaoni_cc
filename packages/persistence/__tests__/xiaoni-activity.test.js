...
test('Xiaoni action stream orders events by global occurred_seq', async () => {
  // Real events carry occurred_seq (the global creation sequence). The 模型请求
  // slice is not an appended item, so it derives to occurred_seq(first output) - 0.5.
  const persistence = createPersistence({
    llmRequestSliceRows: [{
      id: '7', sliceId: 'slice_order', llmCallId: 'llm_order', identityKey: 'xiaoni',
      traceId: 'trace_order', runId: 'run_order', agentTurn: 1, modelName: 'claude-opus-4-6',
      status: 'completed', createdAt: '2026-06-26T14:58:21.084Z', completedAt: '2026-06-26T14:58:21.084Z',
      inputStartIndex: 40, inputEndIndex: 40, outputStartIndex: 41, outputEndIndex: 42,
      tokenUsage: { input_tokens: 100, output_tokens: 5 }, wireResponse: { id: 'resp_order' }
    }],
    agentStackRows: [
      { id: '440', eventId: 'stack:order:input', identityKey: 'xiaoni', stackIndex: 40, occurredSeq: 1000,
        ...
        traceId: 'trace_order', runId: 'run_order', createdAt: '2026-06-26T14:58:21.000Z' },
      { id: '441', eventId: 'stack:order:fc', identityKey: 'xiaoni', stackIndex: 41, occurredSeq: 1001,
        itemKind: 'function_call', toolCallId: 'call_order', llmRequestSliceId: 'slice_order',
        content: { type: 'function_call', call_id: 'call_order', name: 'exec_command', arguments: '{"cmd":"date"}' },
        traceId: 'trace_order', runId: 'run_order', createdAt: '2026-06-26T14:58:21.040Z' },
      { id: '442', eventId: 'stack:order:fco', identityKey: 'xiaoni', stackIndex: 42, occurredSeq: 1002,
        itemKind: 'function_call_output', toolCallId: 'call_order', llmRequestSliceId: 'slice_order',
        content: { type: 'function_call_output', call_id: 'call_order', output: 'done' },
        traceId: 'trace_order', runId: 'run_order', createdAt: '2026-06-26T14:58:21.422Z' }
    ]
  });

  const stream = await persistence.getXiaoniActionStream({ limit: 10 });
  const byId = new Map(stream.items.map((item) => [item.id, item]));
  const input = byId.get('stack:440');
  const slice = byId.get('llm-slice:slice_order');
  assert.ok(input && slice);
...
test('Xiaoni action stream keeps each runtime_input on its own occurred_seq', async () => {
  // With a per-event sequence, each runtime_input simply carries its own
  // occurred_seq — no turn-matching, so overlapping slice input ranges can't
  // make an old input adopt a later turn.
  const persistence = createPersistence({
    agentStackRows: [
      { id: '50', eventId: 'stack:a:input', identityKey: 'xiaoni', stackIndex: 50, occurredSeq: 2000,
        itemKind: 'runtime_input',
      ...
      { id: '52', eventId: 'stack:b:input', identityKey: 'xiaoni', stackIndex: 52, occurredSeq: 2100,
        itemKind: 'runtime_input',
  ...
  const stream = await persistence.getXiaoniActionStream({ limit: 10 });
  const byId = new Map(stream.items.map((item) => [item.id, item]));
  assert.equal(byId.get('stack:50').metadata.orderSeq, 2000);
  assert.equal(byId.get('stack:52').metadata.orderSeq, 2100);
});
