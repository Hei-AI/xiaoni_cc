    ...
    query: async (sql, params = []) => {
      ...
      if (sql.includes('INSERT INTO core_memory_compression_fork_runs')) {
        const row = {
          ...
          trace_id: params[4],
          run_id: params[5],
          read_cutoff_after_stack_index: params[6],
          previous_read_cutoff_after_stack_index: params[7],
      ...
      if (sql.includes('INSERT INTO subconscious_agent_fork_runs')) {
        const row = {
          ...
          trace_id: params[4],
          run_id: params[5],
          notify_queue_message_id: params[6],
          summary_text: params[7],
          artifact: JSON.parse(params[8]),
          error_message: params[9],
          metadata: JSON.parse(params[10]),
          started_at: params[11] || '2026-06-11T00:00:00.000Z',
      ...
      if (sql.includes('INSERT INTO core_memory_compression_fork_tool_executions')) {
        const row = {
          ...
          trace_id: params[13],
          run_id: params[14],
          agent_turn: params[15],
          stack_call_item_id: params[16],
          stack_output_item_id: params[17],
      ...
      if (sql.includes('INSERT INTO llm_request_slices')) {
        const row = {
          ...
          trace_id: params[16],
          run_id: params[17],
          agent_turn: params[18],
          model_name: params[19],
          model_provider: params[20],
          request_format_version: params[21],
          wire_provider_format: params[22],
          processing_time_ms: params[23],
          metadata: JSON.parse(params[24]),
      ...
      if (sql.includes('INSERT INTO codex_provider_usage_events')) {
        const row = {
          ...
          trace_id: params[5],
          run_id: params[6],
          canonical_request: JSON.parse(params[7]),
          wire_request: params[8] ? JSON.parse(params[8]) : null,
          canonical_response: params[9] ? JSON.parse(params[9]) : null,
          wire_response: params[10] ? JSON.parse(params[10]) : null,
          raw_response: params[11] ? JSON.parse(params[11]) : null,
          output_items: JSON.parse(params[12]),
          status: params[13],
          token_usage: JSON.parse(params[14]),
          model_name: params[15],
          model_provider: params[16],
          request_format_version: params[17],
          wire_provider_format: params[18],
          processing_time_ms: params[19],
          metadata: JSON.parse(params[20]),
          created_at: params[21] || '2026-06-11T00:00:00.000Z',
          completed_at: params[22],
          updated_at: '2026-06-11T00:00:00.000Z'
        };
